<?php

class JSBreadCrumbsHooks {

	/**
	 * Implements BeforePageDisplay hook.
	 * See https://www.mediawiki.org/wiki/Manual:Hooks/BeforePageDisplay
	 * Initializes variables to be passed to JavaScript.
	 *
	 * @param OutputPage $output OutputPage object
	 * @param Skin $skin Skin object that will be used to generate the page
	 */
	public static function onBeforePageDisplay( $output, $skin ) {
		$user = $output->getUser();
		if ( !$user->isAllowed( 'read' ) ||
			!$user->getOption( 'jsbreadcrumbs-showcrumbs' ) ) {
			return;
		}

		$vars = [];

		$vars['SiteMaxCrumbs'] = $user->getOption( 'jsbreadcrumbs-numberofcrumbs' );
		$vars['Direction'] = $user->getOption( 'jsbreadcrumbs-direction' );
		$vars['ShowSite'] = (bool)$user->getOption( 'jsbreadcrumbs-showsite' );
		$vars['ShowGlobal'] = (bool)$user->getOption( 'jsbreadcrumbs-showglobal' );

		// Allow localized separator to be overriden
		if ( $GLOBALS['wgJSBreadCrumbsSeparator'] !== '' ) {
			$vars['Separator'] = $GLOBALS['wgJSBreadCrumbsSeparator'];
		} else {
			$vars['Separator'] = wfMessage( 'jsbreadcrumbs-separator' )->escaped();
		}
		$vars['CSSSelector'] = $GLOBALS['wgJSBreadCrumbsCSSSelector'];

		$vars['GlobalMaxCrumbs'] = $GLOBALS['wgJSBreadCrumbsGlobalMaxCrumbs'];

		$vars['LeadingDescriptionHorizontal'] =
			wfMessage( 'jsbreadcrumbs-intro-horizontal',
			$vars['SiteMaxCrumbs'] )->plain();
		$vars['LeadingDescriptionVertical'] =
			wfMessage( 'jsbreadcrumbs-intro-vertical',
			$vars['SiteMaxCrumbs'] )->plain();

		$title = $output->getTitle();
		if ( self::getDisplayTitle( $title, $displayTitle ) ) {
			$pagename = $displayTitle;
		} else {
			$pagename = $title->getPrefixedText();
		}
		$action = Action::getActionName( $output->getContext() );
		if ( $action !== 'view' ) {
			$pagename .= ' [' . wfMessage( $action )->plain() . ']';
		}
		$maxLength = $user->getOption( 'jsbreadcrumbs-maxlength' );
		if ( $maxLength > 0 && strlen( $pagename ) > $maxLength ) {
			$pagename = substr( $pagename, 0, $maxLength ) . '...';
		}
		$vars['PageName'] = $pagename;

		$output->addJSConfigVars( 'JSBreadCrumbs', $vars );
		$output->addModules( 'ext.JSBreadCrumbs' );
	}

	/**
	 * GetPreferences hook
	 *
	 * @param User $user User whose preferences are being modified
	 * @param array &$preferences Preferences description array
	 */
	public static function onGetPreferences( User $user, array &$preferences ) {
		$preferences['jsbreadcrumbs-showcrumbs'] = [
			'type' => 'toggle',
			'label-message' => 'prefs-jsbreadcrumbs-showcrumbs',
			'section' => 'rendering/jsbreadcrumbs'
		];

		$max = $GLOBALS['wgJSBreadCrumbsGlobalMaxCrumbs'];
		$preferences['jsbreadcrumbs-numberofcrumbs'] = [
			'type' => 'int',
			'min' => 1,
			'max' => $max,
			'label-message' => 'prefs-jsbreadcrumbs-numberofcrumbs',
			'section' => 'rendering/jsbreadcrumbs'
		];

		$preferences['jsbreadcrumbs-maxlength'] = [
			'type' => 'int',
			'min' => 0,
			'max' => 100,
			'label-message' => 'prefs-jsbreadcrumbs-maxlength',
			'section' => 'rendering/jsbreadcrumbs'
		];

		$preferences['jsbreadcrumbs-direction'] = [
			'type' => 'select',
			'label-message' => 'prefs-jsbreadcrumbs-direction',
			'options-messages' => [
				'prefs-jsbreadcrumbs-direction-horizontal' => 'horizontal',
				'prefs-jsbreadcrumbs-direction-vertical' => 'vertical'
			],
			'section' => 'rendering/jsbreadcrumbs'
		];

		$preferences['jsbreadcrumbs-showsite'] = [
			'type' => 'toggle',
			'label-message' => 'prefs-jsbreadcrumbs-showsite',
			'section' => 'rendering/jsbreadcrumbs'
		];

		$preferences['jsbreadcrumbs-showglobal'] = [
			'type' => 'toggle',
			'label-message' => 'prefs-jsbreadcrumbs-showglobal',
			'section' => 'rendering/jsbreadcrumbs'
		];
	}

	/**
	 * Get displaytitle page property text.
	 *
	 * @since 1.0
	 * @param Title $title the Title object for the page
	 * @param string &$displaytitle to return the display title, if set
	 * @return boolean true if the page has a displaytitle page property that is
	 * different from the prefixed page name, false otherwise
	 */
	private static function getDisplayTitle( Title $title, &$displaytitle ) {
		$pagetitle = $title->getPrefixedText();
		$title = $title->createFragmentTarget( '' );
		if ( $title instanceof Title ) {
			$values = PageProps::getInstance()->getProperties( $title, 'displaytitle' );
			$id = $title->getArticleID();
			if ( array_key_exists( $id, $values ) ) {
				$value = $values[$id];
				if ( trim( str_replace( '&#160;', '', strip_tags( $value ) ) ) !== '' &&
					$value !== $pagetitle ) {
					$displaytitle = $value;
					return true;
				}
			}
		}
		return false;
	}
}
